---
title: "Clustering (untreated samples)"
output:
...
```{r}
dds <- DESeqDataSetFromTximport(txi.untreated, metadata_untreated, ~Condition1)
```
...
df <- as.data.frame(colData(dds)[,"Condition1"])
annotation <- data.frame(Var1 = metadata_untreated$Condition1, Var2 = metadata_untreated$Condition2)
...
rownames(samplDistMatrix) <- metadata_untreated$Condition1
colnames(samplDistMatrix) <- metadata_untreated$Samples
...
```{r, fig.width=8, fig.height=6}
plotPCA(vsd, intgroup=c("Condition1"))
```

## PCA - by patient and timepoint

```{r, fig.width=12, fig.height=10}
plotPCA(vsd, intgroup=c("Patient", "Condition1"))
```
...
```{r, fig.width=12, fig.height=12}
plotPCA(vsd, intgroup=c("Condition3", "Condition1"))
