...
Note: metadata file has several Condition columns: Condition1 timepoint, Condition2 treatment, Condition3 library prep.
...
```{r}
meta_data <- read.table("metadata.csv", sep=",", header = TRUE)
rownames(meta_data) <- meta_data$Samples
colnames(txi.kallisto$counts) <- rownames(meta_data)
all(rownames(meta_data) %in% colnames(txi.kallisto$counts))
```

```{r}
dds <- DESeqDataSetFromTximport(txi.kallisto, meta_data, ~Condition2)
```
...
df <- as.data.frame(colData(dds)[,"Condition2"])
annotation <- data.frame(Var1 = meta_data$Condition3, Var2 = meta_data$Condition2)
...
rownames(samplDistMatrix) <- meta_data$Condition2
...
plotPCA(vsd, intgroup=c("Patient"))
```
...
## PCA - by patient and treatment

```{r, fig.width=8, fig.height=6}
plotPCA(vsd, intgroup=c("Patient", "Condition2"))
```

## PCA - by individual sample
...
```{r, fig.width=12, fig.height=12}
plotPCA(vsd, intgroup=c("Condition3"))
